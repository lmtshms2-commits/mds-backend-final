...
// تسجيل مستخدم جديد (للصيدليات والشركات والسائقين)
router.post('/register', async (req, res) => {
  try {
    ...
    const userData = { 
      ...
      verified: false // يحتاج موافقة الأدمن
    };
...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!user) return res.status(400).json({ message: 'رقم الجوال أو الدور غير صحيح' });
    if (!user.isActive) return res.status(400).json({ message: 'الحساب غير مفعل، يرجى التواصل مع الإدارة' });
    
    const isValid = await bcrypt.compare(password, user.password);
    if (!isValid) return res.status(400).json({ message: 'كلمة السر خطأ' });
...
// تحديث موقع المستخدم (للصيدليات والشركات)
